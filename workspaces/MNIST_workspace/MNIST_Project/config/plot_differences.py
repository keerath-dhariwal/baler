...
decomp_data = np.load("workspaces\MNIST_workspace\MNIST_Project\output\decompressed_output\decompressed.npz")
clean_decomp_data = decomp_data["data"][:, 0, :, :]
...
for number in numbers:
    ...
    ax.set_title('Root sum square of the differences for every "' +
                 str(number[1]) + '" image')
    ax.set_xlabel("Error value")
    ax.set_ylabel("Count")
    plt.savefig(
